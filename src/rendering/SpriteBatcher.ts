...
export function buildSpriteBatches(
  ...
  for (const npc of npcs) {
    ...
    const npcFeetY = getNPCFeetY(npc.tileY);
  ...
  for (const effect of fieldEffects) {
    ...
    // Determine if this effect renders in front or behind player
    const effectLayer = computeFieldEffectLayer(effect, playerCenterY);

    // Calculate sort key based on layer
    let sortKeyY: number;
    ...
    ySorted.push({
      ...
      type: 'fieldEffect',
      sortKey,
      tileX: Math.floor(effect.worldX / METATILE_SIZE),
