/**
 * Hand-coded story-script handlers for early-game progression.
 *
 * C references:
...
import type { StoryScriptContext } from './StoryScriptContext.ts';
...
export async function executeStoryScript(scriptName: string, ctx: StoryScriptContext): Promise<boolean> {
  ...
  const showExclamationOrDelay = async (mapId: string, localId: string, fallbackFrames = 48): Promise<void> => {
    if (ctx.showEmote) {
      await ctx.showEmote(mapId, localId, 'exclamation', fallbackFrames);
      return;
    }
    await ctx.delayFrames(fallbackFrames);
  };
  ...
  switch (scriptName) {
    ...
    case 'LittlerootTown_MaysHouse_1F_EventScript_PetalburgGymReport': {
      ...
      await showExclamationOrDelay(houseMapId, momLocalId);
    ...
    case 'LittlerootTown_MaysHouse_1F_EventScript_YoureNewNeighbor': {
      ...
      await showExclamationOrDelay(houseMapId, rivalMomLocalId);
    ...
    case 'LittlerootTown_MaysHouse_1F_EventScript_MeetRival2': {
      ...
      await showExclamationOrDelay(houseMapId, rivalLocalId);
